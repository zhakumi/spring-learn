 ...
 *   AbstractBeanDefinition是抽象类，它是众多子类的父类。
 *  前两篇文章讲的BeanDefinition属性值大都保存在AbstractBeanDefinition，并实现了子BeanDefinition通用方法
 *  要点：AbstractBeanDefinition 就是保存了属性值并对属性值进行设置和读取
 */
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	//bean 的作用范围 对应bean 的scope 属性
	@Nullable
	private String scope = SCOPE_DEFAULT;
    //是否抽象 对应bean的abstract
	private boolean abstractFlag = false;
    //是否延迟加载 对用bean的lazy-init
	@Nullable
	private Boolean lazyInit;
    //自动注入模式 对应bean属性autowire
	private int autowireMode = AUTOWIRE_NO;
    //依赖检查 spring 3 之后弃用这个属性
	private int dependencyCheck = DEPENDENCY_CHECK_NONE;
    //用来表示一个bean的实例化依靠另一个Bean先实例化 对应Bean的depend-on
	@Nullable
    ...
    //自动装配出现多个Bean候选者时，将作为首选这，对应Bean的primary
	private boolean primary = false;
    //记录Qualifiler 对应子元素记录qualifiler
	...
	//用法：<bean id = "currentTime" factory-bean = "instanceFactoryBean" factory-method = "createTime" />
	@Nullable
